...
if __name__ == "__main__":
    ...
    templateWeightMap = {}
    for i, tmp in enumerate(ioutil.getTmpList()):
        templateWeightMap[tmp] = weightSumList[i][0]
    print templateWeightMap

    a = sorted(templateWeightMap.items(), lambda x, y: cmp(x[1], y[1]), reverse=True)

    ioutil.writeMatrix(a, SORTED_MATRIXSUM_FILE)
